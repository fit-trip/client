object GeoTrans {
    ...
    fun convert(srctype: Int, dsttype: Int, in_pt: GeoPoint): GeoPoint {
        val tmpPt = GeoPoint()
        val out_pt = GeoPoint()
        if (srctype == GEO) {
            tmpPt.x = D2R(in_pt.x)
    ...
    fun geo2tm(dsttype: Int, in_pt: GeoPoint, out_pt: GeoPoint) {
        val x: Double
        val y: Double
    ...
    fun tm2geo(srctype: Int, in_pt: GeoPoint, out_pt: GeoPoint) {
    ...
    fun getDistancebyGeo(pt1: GeoPoint, pt2: GeoPoint): Double {
        val lat1 = D2R(pt1.y)
        val lon1 = D2R(pt1.x)
        val lat2 = D2R(pt2.y)
        val lon2 = D2R(pt2.x)
	...
	*/
    ...
    private fun transform(srctype: Int, dsttype: Int, point: GeoPoint) {
